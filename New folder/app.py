...
def get_analytics():
    try:
        # 1. Get date range from query params
        ...
        if range_param == 'today':
            ...
            try:
                ...
                if not start_date_str or not end_date_str:
                    return jsonify({"status": "error", "message": "Custom range requires 'start' and 'end' dates."}), 400
                start_date = datetime.strptime(start_date_str, '%Y-%m-%d')
                end_date = datetime.strptime(end_date_str, '%Y-%m-%d')
            except (ValueError, TypeError):
                return jsonify({"status": "error", "message": "Invalid date format. Use YYYY-MM-DD."}), 400
        else:
            return jsonify({"status": "error", "message": "Invalid range parameter."}), 400

        # 2. Get all filenames in the date range
        all_files_to_read = []
        current_date = start_date
        while current_date <= end_date:
            filename = os.path.join(DATA_DIR, f"orders_{current_date.strftime('%Y-%m-%d')}.csv")
            if os.path.exists(filename):
                all_files_to_read.append(filename)
            current_date += timedelta(days=1)

        # 3. Load Menu to get item-to-category mapping
        ...
        # 4. Initialize Analytics Variables
        total_orders = 0
        gross_revenue = 0.0
        cash_total = 0.0
        card_total = 0.0
        ...
        item_revenue_counter = Counter()
        sales_by_hour = defaultdict(float)

        # 5. Read and Aggregate Data from all relevant CSV files
        for filename in all_files_to_read:
            try:
                with open(filename, 'r', newline='', encoding='utf-8') as f_read:
                    reader = csv.DictReader(f_read)
                    for row in reader:
                        try:
                            ...
                            order_timestamp = datetime.strptime(row.get('timestamp'), '%Y-%m-%d %H:%M:%S')
                            sales_by_hour[order_timestamp.hour] += order_total
                            ...
                            app.logger.warning(f"Could not parse row in analytics processing from {filename}: {row}. Error: {e}")
            except Exception as e:
                app.logger.error(f"Failed to process analytics file {filename}: {e}")


        # 6. Calculate Final KPIs and format for response
        ...
        sorted_items_by_qty = item_sales_counter.most_common()
        best_sellers = [{"name": name, "quantity": qty} for name, qty in sorted_items_by_qty[:10]]
        worst_sellers = [{"name": name, "quantity": qty} for name, qty in sorted_items_by_qty[-10:]]
        worst_sellers.reverse()
        
        sorted_items_by_rev = item_revenue_counter.most_common()
        top_revenue_items = [{"name": name, "revenue": total} for name, total in sorted_items_by_rev[:10]]

        # Format sales by hour to ensure all hours are present
        formatted_sales_by_hour = [{"hour": h, "total": sales_by_hour.get(h, 0.0)} for h in range(24)]

        # 7. Construct and Return JSON Response
        analytics_data = {
            ...
            "topRevenueItems": top_revenue_items, # New field
            "salesByHour": formatted_sales_by_hour # New field
        }
